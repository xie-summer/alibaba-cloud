...
public class PaginationInterceptor extends AbstractSqlParserHandler implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        buildSql = concatWhereWrapper(buildSql, page);
     ...
     * @return
     */
    public static String concatWhereWrapper(String originalSql, IPage page) {

        if (page instanceof PageWrapp) {
            PageWrapp pageWrapp = (PageWrapp) page;
            Map condition = pageWrapp.getCondition();
            return geOfCondition(condition, originalSql, page.getCurrent() * page.getSize());
        }
        return originalSql;
    }
    ...
    public static void main(String[] args) {
        ...
        condition.put("id", 1);
        geOfCondition(condition," SELECT * from biz_outwhse_order where name=1",3);
    }

    public static String geOfCondition(Map<String, Object> condition, String originalSql, Object limit) {
        if (CollectionUtils.isEmpty(condition) || condition.size() > 1) {
            return originalSql;
        }
        StringBuilder sql = new StringBuilder(originalSql);
        if (originalSql.toLowerCase().contains("where")) {
            sql.append(String.format(" and  %s  > %s", condition.keySet().iterator().next(), limit));
        } else {
            sql.append(String.format(" where  %s  > %s", condition.keySet().iterator().next(), limit));
        }
        return sql.toString();
    }
